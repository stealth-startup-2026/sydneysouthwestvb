          ...
          {LINKS.map((link) => {
            ...
            return (
              <Link key={link.href} href={link.href} onClick={() => setOpen(false)}
                className={`block px-6 py-3 font-heading text-lg uppercase tracking-wide transition ${active ? "bg-green/20 text-gold" : "text-white hover:bg-white/5 hover:text-gold"}`}>
                {link.label}
              </Link>
            );
          })}
